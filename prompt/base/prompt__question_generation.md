...
生成一個簡答問題，主題為{topic}，要求學生解釋概念。難度：{level}。使用 Chain-of-Thought：先思考知識點，然後生成問題。

生成一個程式碼基底問題，主題為{topic}，要求寫 C++ 或 Python 程式碼。難度：{level}。使用多步驟提示：分解問題步驟。

基於以上問題生成 {n} 個變異，使用不同參數如 {param: e.g., array size}。保持難度 {level}。

生成一個混合問題：先概念，然後簡答，最後程式碼。主題 {topic}，難度 {level}。
